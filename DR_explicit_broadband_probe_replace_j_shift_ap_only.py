...
# -*- coding: utf-8 -*-
"""
explicit probe and object update, jianhua method
@author: Arjun Rana
"""
#%%
import sys
...
parser.add_argument("-arf", "--ap_repl_freq",type=int,default=5,help="probe replacement frequency")
...
file_name = ePIE_struct[0,0].FileName[0]
...
scaling_ratio = np.maximum(pixel_size / pixel_size[best_mode], pixel_size[best_mode] / pixel_size) 
...
if scoop_range > little_area:
    ...
    pad_post = 0
cutoff = iterations / 2
prb_rplment_weight = 0.1 * np.ones(iterations)#np.squeeze(np.minimum([(cutoff**4.0/10.0) / (np.arange(iterations)+1)**4.0], [0.1]))
...
if rank != best_mode:
    ...
    H_bk_best = np.exp(1j * k * -fresnel_dist) * np.exp(-1j*np.pi*llambda[best_mode]
                                            *-fresnel_dist*( u ** 2 + v ** 2))
elif rank == best_mode:
    H_bk = np.exp(1j * k * -fresnel_dist) * np.exp(-1j*np.pi*llambda[rank]
                                            *-fresnel_dist*( u ** 2 + v ** 2))
    H_bk_shifted = np.fft.ifftshift(H_bk)
...
for n in range(iterations):
    ...
    does_ap_replace[n] = np.random.random_sample(1) #np.random.random_sample(n_apert)
     
 #%% main reconstruction loop
if rank == 0: 
    print '=============starting DR explicit (with probe replacement j) reconstruction============='
for itt in range(iterations):
    t0 = time.time()
    w = ws[itt]
    alpha_itt = alpha_itts[itt].copy()
    if (np.mod(itt+1,ap_repl_freq) == 0 and itt >= replace_ap_after 
        and itt <= replace_ap_until):
        probe_replacement_flag = True
        if rank == 0:
            print "probe replacement on"
    else:
...
#        collected_mags = collected_mags.T #for some reason, Allreduce transposes in this context
        ...
        #update object
        u_new = u_old + dt * np.conj(aperture) * diff_exit_wave
        ...
        ap_temp_updated = aperture + ds * np.conj(u_old) * diff_exit_wave 
        ...
        if probe_replacement_flag:
            ...
            if rank != best_mode:
                best_probe = np.empty([little_area,little_area],dtype=complex)
            else:
                best_probe = np.fft.ifftn(np.fft.fftn(aperture) * H_bk_shifted)
            comm.Bcast(best_probe, root=best_mode) 

            if pixel_size[rank] < pixel_size[best_mode]:
                best_probe_cropped = best_probe[scoop_vec][:,scoop_vec]
                Fprobe = pr.my_fft(aperture) * H_bk
                Fprobe[np.ix_(scoop_vec, scoop_vec)] = pr.my_fft(best_probe_cropped).copy()
                probe_replaced = pr.my_ifft(Fprobe)
                probe_replaced = np.fft.ifftn(np.fft.fftn(probe_replaced) * H_fwd)
            elif pixel_size[rank] > pixel_size[best_mode]:
                F_best_probe = pr.my_fft(best_probe)
                F_best_probe_cropped = F_best_probe[scoop_vec][:,scoop_vec]
    ...
    #center probe and object
        if probe_replacement_flag:
            aligned_dict = pr.com_align(best_probe)
            x_shift = np.round(aligned_dict['x_com'] * pixel_size[best_mode] / pixel_size[rank]).astype(int)
            y_shift = np.round(aligned_dict['y_com'] * pixel_size[best_mode] / pixel_size[rank]).astype(int)
            aperture = np.roll(aperture, [-y_shift, -x_shift], [0,1])
            #rrbig_obj = np.roll(big_obj, [-y_shift, -x_shift], [0,1])
    ...
    if (np.mod(itt,10) == 0 and itt > 0):
        ...
        if rank == 0:
            saveString = "DR_explicit_output_probe_replace_j_sao_%s_%s" % (file_name, job_id)
